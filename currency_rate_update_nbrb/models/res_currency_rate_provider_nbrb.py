...
class ResCurrencyRateProviderNBRB(models.Model):
    ...
    service = fields.Selection(
        selection_add=[("NBRB", "National Bank of the Republic of Belarus (NBRB)")],
    ...
    def _obtain_rates(self, base_currency, currencies, date_from, date_to):
        ...
        try:
            ...
            raise UserError(
                self.env._("Error connecting to NBRB API: %s", str(e))
            ) from e
        ...
        try:
            ...
            raise UserError(
                self.env._(
                    "Exchange data format error from "
                    "National Bank of the Republic of Belarus: %s",
                    str(e),
                )
            ) from e
        ...
        try:
            ...
            raise UserError(
                self.env._("Could not parse rate date from NBRB response: %s", str(e))
            ) from e
